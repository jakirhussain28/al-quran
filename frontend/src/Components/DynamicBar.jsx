...
function DynamicBar({ chapters, selectedChapter, onSelect, onVerseJump }) {
  ...
  // Desktop Hover Logic
  // Only open Chapters if we aren't already looking at Verses
  ...
  const toggleChapterView = (e) => {
    if(e.target.closest('.verse-trigger')) return; // Ignore if clicking verse button
    
    // If verse view is open, switch to chapters. Otherwise toggle.
    if (isVerseView) {
        setIsVerseView(false);
        setIsChapterView(true);
    } else {
  ...
  const toggleVerseView = (e) => {
    e.stopPropagation();
    // If chapter view is open, switch to verses. Otherwise toggle.
    if (isChapterView) {
        setIsChapterView(false);
        setIsVerseView(true);
    } else {
  ...
  return (
    <div 
      ...
      onClick={(e) => {
         if (window.innerWidth < 1024) toggleChapterView(e);
      }}
    >
      {/* ANIMATION CONTAINER */}
      <div className={`
      ...
      `}>
        ...
        <div className="h-10 md:h-10.5 w-full flex items-center justify-between px-3 md:px-3 cursor-pointer shrink-0 relative z-20 bg-[#1a1b1d] rounded-[20px]">
          {selectedChapter ? (
            <div className="flex items-center w-full gap-2 md:justify-between">
              ...
              {/* --- RIGHT: Verses Count / Dropdown Trigger --- */}
        ...
        {/* --- 2. DROPDOWN CONTENT AREA --- */}
        {/* We use one container, but swap the content based on which view is active */}
        <div className={`
        ...
        `}>
          ...
          {/* Render CHAPTERS if that mode is active */}
          {isChapterView && (
             <ChapterDropdown 
                chapters={chapters} 
          ...
          {/* Render VERSES if that mode is active */}
